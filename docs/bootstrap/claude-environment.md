...
- Repo: `OMT-Global/home-tv-channel-list`
...
- Auth:
  ...
  - fallback: add a repository secret named `ANTHROPIC_API_KEY`
    ...
    - Keep the Claude workflow out of the required PR check set. The required checks are `CI Gate`, `Build Demo PDF`.
...
- Keep Claude review and automation on GitHub-hosted runners; do not move it onto the self-hosted shell-only fleet.
    ...
    - Repository: `OMT-Global/home-tv-channel-list`
    - Default branch: `main`
