    ...
    - Confirm the repository exists at `OMT-Global/home-tv-channel-list`.
    - Confirm branch protection or rulesets on `main` require one approval and code owner review.
    - Confirm branch protection points at the expected required status checks: `CI Gate`, `Build Demo PDF`.
